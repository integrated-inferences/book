...
Bayesian updating is all about learning. We can see right away from  @eq-Bayes whether we *learned* anything from data $d$. The simplest notion of learning is that our beliefs after seeing $d$ are different than they were before we saw $d$. That is $\Pr(H|d) \neq \Pr(H)$. Or using Equation @eq-Bayes, we have learned something if: 
\index{Learning}
\begin{equation}
...
To consider the simplest scenario, suppose there are two mutually exclusive and exhaustive hypotheses, $H_0$ and $H_1$ and that we place a prior probability of $p$ on $H_1$. Imagine that the available evidence can take on two values, $K=0$ or $K=1$. Likelihoods are described by $\phi_0$ and $\phi_1$, where $\phi_0$ denotes the probability $K=1$ under $H_0$ and $\phi_1$ denotes the probability $K=1$ under $H_1$. @eq-Bayes then becomes:

$$
\Pr(H_1|K=1)=\frac{\phi_1p}{\phi_1p + \phi_0(1-p)}
$${#eq-Bayes2}

and the condition for learning (@eq-learningcondition) reduces to $\phi_1 \neq \phi_0$, so long as $p\in(0,1)$.  
...
Then, using Bayes' rule (@eq-Bayes) we can calculate the updated belief via: 
...
One thing that these calculations demonstrate is that, as a practical matter, we do not have to go through the process of calculating a likelihood to engage in Bayesian updating. If we can directly calculate $\Pr(H,d)$ and $\Pr(d)$, then we can make direct use of @eq-condprob instead of @eq-Bayes.
...
Once queries are defined in terms of causal types, the formation of beliefs, given data $d$, about queries follows immediately from application of @eq-condprob. 
