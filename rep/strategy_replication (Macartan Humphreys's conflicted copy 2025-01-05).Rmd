...
augment_observed_df <- function(df, model)
  make_events(model, n = 0, include_strategy = TRUE) |>
  rbind(collapse_data(df, model)) |>
  mutate(count = as.integer(count))
...
df <-   data.frame(X=c(1,1,1,0,0,0), Y=c(1,1,0,1,0,0))

observed_df <- collapse_data(df, model = make_model("X -> M -> Y"),
                             drop_family = TRUE)
...
augmented_observed_df <- augment_observed_df(df, make_model("X -> M -> Y"))
...
inferences <-
  ...
  function(model_list, 
           observed = observed_df,  augmented_observed = augmented_observed_df, data_function = make_data_list, 
           subset = TRUE, ...)
    ...
    model_list %>%
     ...
     lapply(function(model) {
      ...
      data_list <- data_function(observed, model)[subset]
...
illustration <- inferences(model_list[1], subset = 1)
...
write_rds(case_selection_1, paste0(path_cs, "/Case_selection_1_fig_2025.rds"))
...
write_rds(case_selection_2, paste0(path_cs, "/Case_selection_2_fig_2025.rds"))
...
if(FALSE)
results %>%
   filter(Model == "Chain" & query == "ATE" & deep_n ==100 & wide_n == 1600) |>
   ggplot(aes(sd^2)) +
   geom_histogram() + facet_wrap(file ~.)

# Suggests variation coming from some extemes
if(FALSE)
results %>%
   filter(Model == "Chain" & Query == "ATE" & deep_n ==100 & wide_n == 1600) |>
  mutate(t = scale(sd^2)) |>
...
results <- 
  results |>
  group_by(query, wide_n, deep_n, Model) |>
  mutate(
...
write_rds(short, paste0(path_wd, "/14_wide_deep_fig_book.rds")) 

write_rds(wide_deep_fig_book, paste0(path_wd, "/wide_deep_data.rds")) 
