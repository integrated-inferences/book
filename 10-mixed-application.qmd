...
#| label: dcaused0
#| echo :  FALSE
#| fig.cap :  "Priors and posteriors on the probability that inequality caused or prevented democratization given different kinds of observed case-level data. The error bars show 95 percent credibility intervals."
#| eval :  FALSE
#| include : FALSE

ch09_queryresults_queries  |> bind_rows(.id = "case") |>
  mutate(using = ifelse(grepl("priors", case), "Priors", "Posteriors"),
         query = ifelse(grepl("result", case), "Inequlity caused democratization", "Inequality prevented democratization"),
         Given = factor(Given, rev(Given_labels), rev(names(Given_labels))))  |> 
  ggplot(aes(mean, Given, shape = using)) + 
  geom_point(position=position_dodge(.5)) + 
  theme_bw() +
  geom_errorbarh(aes(xmin=cred.low, xmax=cred.high), 
                size=.2, height = .1,
                position=position_dodge(.5)) +
  facet_grid(query~., scales = "free_y") + xlab("Estimate")
...
if(run)
  ...
  ggplot(aes(mean, Y, shape = Given)) + geom_point(position = ggstance::position_dodgev(.5)) + ylab("Nodal type for Y") + xlab("Posterior") + theme_bw() + 
