...
#| tbl-cap: 'Data (snippet) derived from Haggard and Kaufman (2012)'
#| echo: false
#| warning: false
data <- CausalQueries::democracy_data  
kabble(head(data), booktabs = TRUE)
...
#| tbl-cap: 'Pairwise correlations in the democracy-inequality (PIMD) data. P = Pressure, I = Inequality, M = Mobilization, D = Democratization'
#| echo: false
#| warning  :  FALSE
#| message: false
...
x1 <- pimd_queries |>
  lapply(function(a) data.frame(p = as.vector(a[,1]))) |>
  bind_rows(.id = "Q") |>
...
```{r}
#| label: tbl-data82RST
#| tbl-cap: 'Data (snippet) derived from Rodrik, Subramanian, and Trebbi (2004)'
#| echo: false
#| warning: false
...
RST %>%
  ...
  head() |>
  kabble( booktabs = TRUE, align = c("l", "c", "c", "c", "c"))
...
#| tbl-cap: 'Pairwise correlations in the growth-institutions data. D = Distance from the Equator, M = Settler Mortality, R = Institutional Rule of Law, Y = Growth'
#| echo: false
#| warning  :  FALSE
#| message: false
...
```{r}
#| label: tbl-RSTATE
#| tbl-cap: 'Posterior beliefs about the average effect of R on Y given M and D.'
#| echo: false
bind_rows(
...
kabble(digits = 2)  # |>  kable_styling(latex_options = "scale_down")
```
...
#| tbl-cap: 'Direct and indirect effects of settler mortality on growth'
#| echo: false
...
#| tbl-cap: 'Direct and indirect effects of settler mortality on growth for a case with high mortality, weak institutions, and low growth'
#| echo: false
