...
```{r}
#| label: tbl-HJ-T-13-1
#| tbl-cap: 'Beliefs about data outcomes under different paramater values (X=0 cases omitted).'
#| echo: false
df |> filter(X!=0) |>
  arrange(X, Y, K) |>
...
#| tbl-cap: 'The data we start with. If we are interested in whether $X$ caused $Y$ in case $A$, are we better gathering data on $M for$ in case $A$ or on $K$ in case $B$?'
#| echo: false
...
#| echo: false
#| tbl-cap: "Expected data and projected inferences on effects for case A given one additional clue"


results_a %>%
  select(Quantity,`Best guess` = mean, Uncertainty = variance) %>%
  kabble(digits = 3,  align = c("l", "c", "c"))
...
#| tbl-cap: 'Expected data and projected inferences on effects for case B given one additional clue'
#| echo: false
...
#| tbl-cap: 'Each column shows a possible distribution of data that can be generated from a given strategy. We calculate the probability of each data possibility, given the data seen so far, and the posterior variance associated with each one.'
#| echo: false
#| warning: false  
...
#| tbl-cap: 'Expected posterior variances'
#| echo: false
...
read_rds("saved/13_case_selection_1.rds") 
...
read_rds("saved/13_case_selection_2.rds") 
