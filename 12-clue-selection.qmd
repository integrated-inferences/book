...
kabble(caption="Inferences on whether $X$ has a positive effect on $Y$, given different data patterns under different clue strategies.", row.names = FALSE, digits = 3)
```
...
```{r}
#| label: fig-HJ-F-12-5a
#| echo:  false
#| fig.cap :  "Revised model: What you expect to find and what you expect to learn from different strategies. Size of the dots indicates probability of observing the data pattern given the search strategy."
#| fig.width: 10
#| fig.height: 6
#| eval :  FALSE
#| include :  FALSE

read_rds("saved/12_pimd_strategies_r.rds") |>
  # gather("var", "val", -data_type, -Given, -strategy)|>
  mutate(strategy = factor(strategy, c("MP", "M-", "-P", "--"),
                           c("M & P", "M only", "P only", "Prior \n (no clue data)"))) %>%
  ggplot(aes(`posterior variance`, data_type, size = probability)) + facet_grid( strategy ~Given, scales = "free") + geom_point() + ylab("Clues") + xlab("")
...
Figure  @fig-HJ-F-12-5 shows how our uncertainty would change, in expectation, under different clue selection strategies, comparing a scenario in which we alter the query only, without changing the model, to a scenario in which we alter both the query and the model. 
