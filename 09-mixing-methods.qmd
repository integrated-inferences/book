...
#| tbl-cap: 'Beliefs over two states of the world, where information on a new case leads to updating about the state of the world'
#| echo: false
#| 
...
```{r}
#| label: tbl-HJ-T-9-5
#| tbl-cap: 'Posteriors on potential outcomes for non binary model'
#| echo: false
#| warning: false
if(run) 
...
```{r}
#| label: tbl-merror
#| tbl-cap: 'Inferences on effects on true Y given measurement error (true ATE = .6)'
#| echo: false
...
#| tbl-cap: 'Inferences on effects on true Y given two noisy measures  (true ATE = .6)'
#| echo: false
set.seed(1)
...
We simulate 100 observations from this data-generating process and then update a model (with flat priors over all nodal types). Results are presented in @tbl-HJ09-T10-spillovers
...
```{r}
#| label: tbl-HJ09-T10-spillovers
#| tbl-cap: "Spillover queries"
#| echo: FALSE
#| 
...
```{r code-sec-9Ap, eval = FALSE, echo = TRUE}
query_model(model,
            query = "Y[X=1]> Y[X=0]",
            given = c("X==1 & Y==1", 
                      "X==1 & Y==1 & M==1",
...
#| label: tbl-HJ09-11-1
#| echo: false
#| tbl-cap: "Querying an updated model"
#| 
if(run)
  query_model(model,
            query = "Y[X=1]> Y[X=0]",
...
Results are shown in @tbl-HJ09-11-1. We see that $M$ is informative (particularly when $M=0$) about causal effects in a new case, given our observation of processes in previous cases. The key thing here is that the informativeness of $M$ for the new case is justified by the updating of the original model~--~a model that itself contained no assumptions about whether or how effects passed through $M$.

### Replication of @chickering1996clinician Lipid Analysis.

\index{Replication study} \index{Chickering, David M.} \index{Pearl, Judea}

@chickering1996clinician assess the problem of drawing inferences in the presence of imperfect compliance. They use data that look like those in @tbl-HJ-09-CPdata.\index{Pearl, Judea} \index{Chickering, David M.}

```{r} 
#| label: tbl-HJ-09-CPdata
#| echo: FALSE
#| tbl-cap: "Lipid study data from Chickering and Pearl (1996). Note data are reported in compact form with counts of events, assuming a data strategy in which data are sought on all nodes (ZXY)"
data <- data.frame(
...
#| label: tbl-HJ-T-9-12
#| echo: false
#| tbl-cap: "Replication of Chickering and Pearl (2007)."
...
```{r}
#| label: tbl-HJ-T-9-13
#| tbl-cap: 'Case level counterfactual inference following model updating (replication of Chickering and Pearl 1997).'
#| echo: false
...
```{r}
#| label: tbl-HJ-T-9-14
#| tbl-cap: 'Probability distribution over two parameter vectors'
#| echo: false
...
df |>
  kabble(col.names = c("$\\lambda$", "$p(\\lambda)$", "$\\lambda^X_0$", "$\\lambda^X_1$", "$\\lambda^M_{01}$", "$\\lambda^M_{10}$", "$\\lambda^Y_{01}$", "$\\lambda^Y_{10}$"), align = "c", digits = 4,
         escape = FALSE)
