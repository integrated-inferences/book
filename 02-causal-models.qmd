...
$X_2$. \index{Causal types} The full set is represented in @tbl-HJ-T-2-3, which also illustrates how we read types off of
...
information needed to construct potential outcomes (see
...
| Causal type |  $\theta$  | A causal type is a concatenation of nodal types, one for each node. Example: $(\theta^X_0, \theta^Y_{00})$ is a causal type that has $X=0$ and that has $Y=0$ no matter what the value of $X$. Types like this are written in code in `CausalQueries` as `X.0Y00`. |
...
common cause. Thus, for instance, the graph in @fig-HJ-F-2-1 implies that $I$ and $E$ have no common cause. If we
...
affect multiple nodes in the model.* Indeed, $\theta^I$ in @fig-HJ-F-2-1 already implicitly captures all factors that affect
...
might be operating between two causally linked variables. In 
...
path of arrows pointing in the same direction. In Panel 1 of 
