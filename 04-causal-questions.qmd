...
The *probability* that $X_1=1$ caused $Y=1$ *given* $X_1=1, X_2=1$, and $Y=1$ is the probability of these four types divided by the probability that $Y=1$ given $X_1=1$ and $X_2=1$. The latter probability is the probability of being in the last eight rows of  @tbl-HJ-T-2-3, which are the eight $Y$-nodal types consistent with the observation of $X_1=1$, $X_2=1$, and $Y=1$. 
...
Code ~~to~~`r colorize("for")` answering these two simple queries is shown below and the output is shown in @tbl-HJ-T-4-1 (one row for each causal type).
...
#| label: tbl-HJ-T-4-1
#| echo: false
#| tbl-cap: "Set of causal types in the model that satisfy q1 and q2 along with the probability of the type."
kabble(df, booktabs = TRUE)
```
