...
#| fig.cap :  "Possible inferences on `X caused Y` given observation of M: 0 and M: 1 given different possible models. The highest inference when M: 0 is lower than the lowest inference when M: 1"
#| echo:  false
#| message: FALSE
#| warning: FALSE
...
rbind(
  ...
  kabble(digits = 3, caption =  "Regression coefficients for $M$  and $R$ on $D$ (estimated separately).")
...
```{r}
#| label: fig-HJ-F-16-10-excluded-2
#| echo:  false
#| eval :  FALSE
#| message :  FALSE
#| include :  FALSE
if(run)
 make_predictions(RST_3, "saved/16_replicates_1.rds")

plot_distribution("saved/16_replicates_1.rds")
```

```{r}
#| label: fig-HJ-F-16-10-excluded-3
#| echo:  false
#| eval :  FALSE
#| message :  FALSE
#| include :  FALSE
if(run){
   ...
   RST_3b <- update(model, data = RST |> uncount(4))
   make_predictions(RST_3b, "saved/16_replicates_3b.rds", n = nrow(RSB)*4)
}

plot_distribution("saved/16_replicates_3b.rds")
```
